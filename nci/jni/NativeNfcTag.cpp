...
static jint nativeNfcTag_doConnect(JNIEnv*, jobject, jint targetHandle) {
...
#if (NXP_EXTNS == TRUE)
  LOG(ERROR) << StringPrintf(
      "%s:  doConnect sCurrentConnectedTargetProtocol %x "
      "sCurrentConnectedTargetType %x",
...
#endif
  LOG(INFO) << StringPrintf("%s: TargetType=%d, TargetProtocol=%d", __func__,
  ...
  if (sCurrentConnectedTargetType == TARGET_TYPE_ISO14443_3A ||
    ...
    if (sCurrentConnectedTargetProtocol != NFC_PROTOCOL_MIFARE) {
      ...
      {
        LOG(DEBUG) << StringPrintf(
            "%s: switching to tech: %d need to switch rf intf to frame",
            __func__, sCurrentConnectedTargetType);
  ...
  static int reSelect(tNFA_INTF_TYPE rfInterface, bool fSwitchIfNeeded) {
    ...
    do {
      ...
      if (NfcTag::getInstance().isNdefDetectionTimedOut()) {
        LOG(DEBUG) << StringPrintf("%s: ndef detection timeout; break",
                                   __func__);
...

      {
        ...
        if (sReconnectEvent.wait(1000) == false)  // if timeout occurred
        {
          ...
          if (nfcTagExtns.processNonStdTagOperation(
            ...
            LOG(INFO) << StringPrintf("%s: Non-standard reconnect %u", __func__,
                                      rfInterface);
#endif
          break;
        }
      ...
      LOG(DEBUG) << StringPrintf("%s: select completed; sConnectOk=%d",
                                 __func__, sConnectOk);
    ...
    if (nfcTagExtns.processNonStdTagOperation(
        ...
        NfcTagExtns::TAG_STATUS_SUCCESS)
      LOG(INFO) << StringPrintf("%s: clear Non-std tag state; status=%d",
                                __func__, rVal);
#endif
    sRfInterfaceMutex.unlock();
  ...
  *******************************************************************************/
  static bool switchRfInterface(tNFA_INTF_TYPE rfInterface) {
    ...
    LOG(DEBUG) << StringPrintf("%s: new rf intf = %d, cur rf intf = %d",
                               __func__, rfInterface, sCurrentRfInterface);
    ...
    do {
      {
        ...
        if (status != NFA_STATUS_OK) {
          LOG(ERROR) << StringPrintf("%s: fail send; error=%d", __func__,
                                     status);
          break;
        }
      ...
      if ((natTag.getProtocol() == NFA_PROTOCOL_T2T) &&
          natTag.isT2tNackResponse(sRxDataBuffer.data(),
                                   sRxDataBuffer.size())) {
        isNack = true;
      }
  ...
  *******************************************************************************/
  void nativeNfcTag_doCheckNdefResult(tNFA_STATUS status, uint32_t maxSize,
    ...
    // capable/formated/read only */ #define RW_NDEF_FL_FORMATABLE 0x10    /*
  ...
  **
  *******************************************************************************/
  static jint nativeNfcTag_doCheckNdef(JNIEnv * e, jobject o,
                                       jintArray ndefInfo) {
    tNFA_STATUS status = NFA_STATUS_FAILED;
    jint* ndef = NULL;
    ...
    if (NfcTagExtns::getInstance().processNonStdTagOperation(
            TAG_API_REQUEST::TAG_CHECK_NDEF_API,
            TAG_OPERATION::TAG_SKIP_NDEF) != NfcTagExtns::TAG_STATUS_STANDARD) {
      ndef = e->GetIntArrayElements(ndefInfo, 0);
      ndef[0] = 0;
      ndef[1] = NDEF_MODE_READ_ONLY;
      e->ReleaseIntArrayElements(ndefInfo, ndef, 0);
    ...
    if (sem_wait(&sCheckNdefSem)) {
      LOG(ERROR) << StringPrintf(
          "%s: Failed to wait for check NDEF semaphore (errno=0x%08x)",
          __func__, errno);
      goto TheEnd;
    }
  ...
  static jboolean nativeNfcTag_doPresenceCheck(JNIEnv*, jobject) {
    ...
    if (NfcTag::getInstance().mTechLibNfcTypes[handle] == NFA_PROTOCOL_T3BT) {
      ...
      int timeout = NfcTag::getInstance().getTransceiveTimeout(
          sCurrentConnectedTargetType);
      LOG(INFO) << StringPrintf("%s: enter; timeout = %d", __func__, timeout);
    ...
    if (NfcTag::getInstance().mTechLibNfcTypes[0] == NFA_PROTOCOL_T3BT) {
      ...
      status = NFA_SendRawFrame(T3btPresenceCheckCmd, bufLen,
                                NFA_DM_DEFAULT_PRESENCE_CHECK_START_DELAY);
      if (status != NFA_STATUS_OK) {
        LOG(ERROR) << StringPrintf("%s: fail send; error=%d", __func__, status);
      } else
        waitOk =
            sTransceiveEvent.wait(NfcTag::getInstance().getTransceiveTimeout(
  ...
  **
  *******************************************************************************/
  static jboolean nativeNfcTag_doIsNdefFormatable(
      JNIEnv * e, jobject o, jint /*libNfcType*/, jbyteArray, jbyteArray,
      jbyteArray) {
    ...
    if (NFA_PROTOCOL_T1T == protocol || NFA_PROTOCOL_T5T == protocol ||
      ...
      isFormattable =
          NfcTag::getInstance().isFelicaLite() ? JNI_TRUE : JNI_FALSE;
    ...
    LOG(DEBUG) << StringPrintf("%s: is formattable=%u", __func__,
                               isFormattable);
  ...
  **
  *******************************************************************************/
  static jboolean nativeNfcTag_doIsIsoDepNdefFormatable(
      JNIEnv * e, jobject o, jbyteArray pollBytes, jbyteArray actBytes) {
  ...
  **
  ** Description:     Receive the result of making a tag read-only. Called by
  *the
  **                  NFA_SET_TAG_RO_EVT.
  **                  status: Status of the operation.
  **
  ...
  **
  *******************************************************************************/
  static jboolean nativeNfcTag_doMakeReadonly(JNIEnv * e, jobject o,
                                              jbyteArray) {
    jboolean result = JNI_FALSE;
    tNFA_STATUS status;
    ...
    if (sem_init(&sMakeReadonlySem, 0, 0) == -1) {
      LOG(ERROR) << StringPrintf(
          "%s: Make readonly semaphore creation failed (errno=0x%08x)",
          __func__, errno);
      return JNI_FALSE;
    }
