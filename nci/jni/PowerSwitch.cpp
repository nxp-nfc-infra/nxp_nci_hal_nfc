  ...
  if (status == NFA_STATUS_OK) {
    ...
    if (mPwrConfstatus != NFA_STATUS_OK) {
      LOG(ERROR) << StringPrintf(
          "%s: Failed to set POWER CONFIGURATION. "
          "Invalid input power configuration",
          __FUNCTION__);
    } else {
      LOG(INFO) << StringPrintf("%s: set Power configuration successful",
                                __func__);
...
void PowerSwitch::deviceManagementCallback(uint8_t event,
  ...
  switch (event) {
    ...
    case NFA_DM_SET_CONFIG_EVT:  // result of NFA_setDynamicPowerConfig
      LOG(INFO) << StringPrintf("%s: NFA_DM_SET_CONFIG_EVT", __func__);
      {
    ...
    case NFA_DM_GET_CONFIG_EVT: /* Result of NFA_GetConfig */
      LOG(INFO) << StringPrintf("%s: NFA_DM_GET_CONFIG_EVT", __func__);
      {
