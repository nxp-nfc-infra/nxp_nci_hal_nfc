...
tNFA_STATUS NxpPropCmd_send(uint8_t* pData4Tx, uint8_t dataLen,
  ...
  LOG(INFO) << StringPrintf("%s: prop cmd being txed", __func__);
  ...
  if (status == NFA_STATUS_OK) {
    LOG(INFO) << StringPrintf("%s: Success NFA_SendNxpNciCommand", __func__);
...
void nfaVSCNtfCallback(uint8_t event, uint16_t param_len, uint8_t* p_param) {
  (void)event;
  LOG(INFO) << StringPrintf("%s: event = 0x%02X", __func__, event);
