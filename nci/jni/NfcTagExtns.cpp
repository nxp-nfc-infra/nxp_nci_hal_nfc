...
void NfcTagExtns::initialize() {
  ...
  if (NfcConfig::hasKey(NAME_NXP_NON_STD_CARD_TIMEDIFF)) {
    ...
    LOG(INFO) << StringPrintf("%s: Non std card", __func__);
    for (size_t i = 0; i < timeDiff.size(); i++) {
      mNonStdCardTimeDiff.at(i) = timeDiff.at(i) * TIME_MUL_100MS;
      LOG(INFO) << StringPrintf("%s: timediff[%zu] = %d", __func__, i,
                                mNonStdCardTimeDiff.at(i));
    }
  } else {
    LOG(INFO) << StringPrintf("%s: timediff not defined taking default",
                              __func__);
  }
...
void NfcTagExtns::processNonStdNtfHandler(EVENT_TYPE event,
  ...
  LOG(INFO) << StringPrintf("%s: processNonStdNtfHandler event :%d ", __func__,
                            event);
...
void NfcTagExtns::processActivatedNtf(tNFA_CONN_EVT_DATA* data) {
  ...
  if ((data->activated.activate_ntf.protocol != NFA_PROTOCOL_NFC_DEP) &&
    ...
    if (getActivatedMode() == TARGET_TYPE_ISO14443_3B) {
      LOG(INFO) << StringPrintf("%s: NFA_ACTIVATED_EVT: received typeB NFCID0",
                                __func__);
      updateNfcID0Param(
...
void NfcTagExtns::resetMfcTransceiveFlag() {
  if (!isNonStdCardSupported) {
    LOG(INFO) << StringPrintf("%s:Non standard support disabled", __func__);
    return;
  }
  isMfcTransceiveFailed = false;
}
...
bool NfcTagExtns::checkAndClearNonStdTagState() {
  ...
  if ((tagState & TAG_NON_STD_SAK_TYPE) ||
    ...
    tagState &= ~TAG_NON_STD_T2T_SAK_TYPE;
    LOG(INFO) << StringPrintf("%s: non std Tag", __func__);
    ret = true;
  }
  return ret;
}
...
void NfcTagExtns::storeNonStdTagData() {
  ...
  if (ret == -1) {
    LOG(ERROR) << StringPrintf("Log : clock_gettime failed");
    clearNonStdTagData();
  } else {
    ...
    LOG(INFO) << StringPrintf("%s: %u is stored", __func__,
                              nonStdTagInfo.rf_disc_id);
  }
}
...
void NfcTagExtns::processtagSelectEvent(tNFA_CONN_EVT_DATA* data) {
  ...
  if (isMfcTransFailed() && data->status != NFA_STATUS_OK) {
    ...
    if (IS_MULTIPROTO_MFC_TAG()) {
      ...
      LOG(INFO) << StringPrintf("%s: Non STD MFC sequence1", __func__);
      int ret = clock_gettime(CLOCK_MONOTONIC, &LastDetectedTime);
      if (ret == -1) {
        LOG(ERROR) << StringPrintf("Log : clock_gettime failed");
      }
...
void NfcTagExtns::processDiscoveryNtf(tNFA_CONN_EVT_DATA* data) {
  ...
  LOG(INFO) << StringPrintf("%s:Non standard support enabled", __func__);
  if (discovery_ntf.rf_tech_param.param.pa.sel_rsp == NON_STD_CARD_SAK) {
    // Non Standard Transit => ISO-DEP
    LOG(INFO) << StringPrintf("%s:Non standard Transit => change to ISO-DEP",
                              __func__);
    ...
    if ((discovery_ntf.protocol == NFC_PROTOCOL_MIFARE) &&
        isSkipNdefRequired(discovery_ntf.rf_tech_param.param.pa.nfcid1)) {
      tagState |= TAG_MFC_NON_STD_TYPE;
...
bool NfcTagExtns::shouldSkipProtoActivate(tNFC_PROTOCOL protocol) {
  ...
  if ((protocol == NFA_PROTOCOL_ISO_DEP) &&
    ...
    LOG(INFO) << StringPrintf("%s: %u is skipped", __func__,
                              act_ntf.rf_disc_id);
...
bool NfcTagExtns::isTagDetectedInRefTime(uint32_t reference) {
  ...
  if (timediff < reference) {
    LOG(INFO) << StringPrintf("%s: Non standard MFC tag detected, sequence-2",
                              __func__);
    isNonStdCard = true;
  }
  return isNonStdCard;
...
void NfcTagExtns::updateNonStdTagState(uint8_t protocol,
  ...
  if (protocol == NFC_PROTOCOL_MIFARE) {
    ...
    if ((tagState & TAG_MFC_NON_STD_TYPE) &&
      ...
      tagState |= TAG_SKIP_NDEF_TYPE;
    } else if (tagState & TAG_SKIP_NDEF_TYPE) {
      LOG(INFO) << StringPrintf("%s: tagState = %d", __func__, tagState);
    } else {
      clearNonStdMfcState();
    }
  ...
  if (getActivatedProtocol() == NFA_PROTOCOL_T2T ||
    ...
    if (status != NFA_STATUS_OK) {
      ...
      if (android::sTransceiveEvent.wait(100) == false) {
        ret = TAG_STATUS_FAILED;
        LOG(ERROR) << StringPrintf("%s: timeout on HALTB", __func__);
      }
    }
...
tTagStatus NfcTagExtns::performTagDeactivation() {
  ...
  if (tagState & TAG_CASHBEE_TYPE) {
    ...
    LOG(INFO) << StringPrintf("%s: Deactivate to IDLE", __func__);
    ...
    LOG(INFO) << StringPrintf("%s: deactivate to sleep", __func__);
    if (NFA_STATUS_OK !=
...
tTagStatus NfcTagExtns::updateTagState() {
  ...
  if (NfcTag::getInstance().getActivationState() == NfcTag::Idle) {
    LOG(INFO) << StringPrintf("%s: Tag is in IDLE state", __func__);
    ...
    if ((NfcTag::getInstance().mActivationParams_t.mTechLibNfcTypes ==
      ...
      LOG(INFO) << StringPrintf("%s: CashBee Detected", __func__);
      ret = TAG_STATUS_PROPRIETARY;
    }
  }
  return ret;
...
tTagStatus NfcTagExtns::performTagReconnect() {
  ...
  if (tagState & TAG_CASHBEE_TYPE) {
    ret = TAG_STATUS_PROPRIETARY;
    LOG(INFO) << StringPrintf("%s: Start RF discovery", __func__);
    if (!(tagState & TAG_ISODEP_DEACTIVATE_FAIL) &&
...
void NfcTagExtns::updateNfcID0Param(uint8_t* nfcID0) {
  LOG(INFO) << StringPrintf("%s: nfcID0 =%X%X%X%X", __func__, nfcID0[0],
                            nfcID0[1], nfcID0[2], nfcID0[3]);
  memcpy(mNfcID0, nfcID0, 4);
}
/******************************************************************************
**
** Function:        isSkipNdefRequired
**
** Description:     Update check the UID of tags
** Returns:         TRUE, if  UID matched
**                  FALSE, if UID not matched
**
*******************************************************************************/
bool NfcTagExtns::isSkipNdefRequired(uint8_t* receivedUid) {
  bool misSkipNdefRequired = false;
  uint8_t refUid[4] = {0};
  uint8_t referenceUid1[] = {0x14, 0x90, 0x62, 0x3C};
  uint8_t referenceUid2[] = {0x16, 0x47, 0x89, 0x24};
  uint8_t lenUid1 = sizeof(referenceUid1) / sizeof(uint8_t);
  uint8_t lenUid2 = sizeof(referenceUid2) / sizeof(uint8_t);
  memcpy(refUid, receivedUid, 4);
  uint8_t lenRecUid = sizeof(refUid) / sizeof(uint8_t);
  if (lenUid1 != lenRecUid) {
    return misSkipNdefRequired;
  }
  // Linearly compare elements
  for (int i = 0; i < lenUid1; i++) {
    if (refUid[i] != referenceUid1[i]) {
      misSkipNdefRequired = false;
      break;
    }
    misSkipNdefRequired = true;
  }
  if (misSkipNdefRequired) {
    return misSkipNdefRequired;
  }
  for (int i = 0; i < lenUid2; i++) {
    if (refUid[i] != referenceUid2[i]) {
      return false;
    }
    misSkipNdefRequired = true;
  }
