...
jboolean NativeT4tNfcee::t4tClearData(JNIEnv* e, jobject o) {
  ...
  switch (t4tNfceeStatus) {
    ...
    case ERROR_NFC_NOT_ON:
      /*NFC is OFF*/
      // if (android::nfcManager_doPartialInitialize(e, o,
...
tNFA_STATUS NativeT4tNfcee::openConnection() {
  ...
  LOG(INFO) << StringPrintf("%s: Exit status = 0x%02x", __func__, status);
  return status;
}
...
void NativeT4tNfcee::t4tReadComplete(tNFA_STATUS status, tNFA_RX_DATA data) {
  ...
  if (status == NFA_STATUS_OK) {
    if (data.len > 0) {
      ...
      LOG(INFO) << StringPrintf("%s: Read Data len new: %d ", __func__,
                                data.len);
    }
  }
  SyncEventGuard g(mT4tNfcEeRWEvent);
...
void NativeT4tNfcee::eventHandler(uint8_t event,
  ...
  switch (event) {
    case NFA_T4TNFCEE_EVT:
      LOG(INFO) << StringPrintf("%s: NFA_T4TNFCEE_EVT", __func__);
      {
    ...
    case NFA_T4TNFCEE_READ_CPLT_EVT:
      LOG(INFO) << StringPrintf("%s: NFA_T4TNFCEE_READ_CPLT_EVT", __func__);
      t4tReadComplete(eventData->status, eventData->data);
      break;

    case NFA_T4TNFCEE_WRITE_CPLT_EVT:
      LOG(INFO) << StringPrintf("%s: NFA_T4TNFCEE_WRITE_CPLT_EVT", __func__);
      t4tWriteComplete(eventData->status, eventData->data);
      break;

    case NFA_T4TNFCEE_CLEAR_CPLT_EVT:
      LOG(INFO) << StringPrintf("%s: NFA_T4TNFCEE_CLEAR_CPLT_EVT", __func__);
      t4tClearComplete(eventData->status);
      break;

    default:
      LOG(INFO) << StringPrintf("%s: unknown Event", __func__);
      break;
  }
}
...
**
** Description:     Set/Reset the lock bit for contact or/and contact less NDEF
*files.
**
** Parameter:       param_val: Reference to a value which shall be modified by
*this API
...
**
*******************************************************************************/
bool NativeT4tNfcee::doChangeT4tFileWritePerm(uint8_t& param_val,
                                              const bool& lock) {
  bool status = false;
  if (lock) { /* Disable the lock bit*/
    if (param_val & (1 << MASK_LOCK_BIT)) {
      param_val &=
          ~(1 << MASK_LOCK_BIT); /* Reset bit6 to disable write permission */
      status = true;
    } else {
      LOG(INFO) << StringPrintf("%s: Lock bit is already disable", __func__);
    }
  } else { /* Enable the lock bit*/
    if (!(param_val & (1 << MASK_LOCK_BIT))) {
      param_val |=
          (1 << MASK_LOCK_BIT); /* Set bit6 to enable write permission */
      status = true;
    } else {
      LOG(INFO) << StringPrintf("%s: Lock bit is already set", __func__);
    }
  }
  return status;
}
...
**
** Parameter:       uint8_t& clNdefFileValue: reference variable to hold value
*of
**                                        contactless (A095) tag
**                  uint8_t& cNdefFileValue : reference variable to hold value
*of
**                                        contact (A110) tag
...
**
*******************************************************************************/
bool NativeT4tNfcee::doGetT4tConfVals(uint8_t& clNdefFileValue,
                                      uint8_t& cNdefFileValue) {
  tNFA_PMID t4tNfcEeNdef[] = {
  ...
  status = android::getConfig(&rspLen, configValue, NXP_NFC_NUM_PARAM_T4T_NFCEE,
                              t4tNfcEeNdef);
...
bool NativeT4tNfcee::doLockT4tData(JNIEnv* e, jobject o, bool lock) {
  LOG(INFO) << StringPrintf("%s: enter %d", __func__, lock);
  ...
  if (!doGetT4tConfVals(clNdefFileValue, cNdefFileValue)) return false;
...
bool NativeT4tNfcee::isNdefWritePermission() {
  ...
  LOG(INFO) << StringPrintf("%s: Exit 0x%lx", __func__, num);
